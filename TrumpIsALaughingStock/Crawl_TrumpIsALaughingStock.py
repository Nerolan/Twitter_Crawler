...
credentials = pd.read_csv("../credentials_patrickStrobel.csv", sep=";")
...
def run():
    ...
    print(result)
    time.sleep(225)
    get_tweets_by_hashtags("#TrumpIsALaughingStock").to_csv("TrumpIsALaughingStock.csv", mode='a', header=False, index=False)
    run()
